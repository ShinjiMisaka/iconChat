...
class Room4ViewController: UIViewController,UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate,MFMailComposeViewControllerDelegate  {
    ...
    var number : Int = 0
    let number4Ref = Database.database().reference().child("numbers4")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //通報ボタン
        mailButton.addTarget(self, action: #selector(Room4ViewController.startMailer(sender:)), for: .touchUpInside)
        self.view.addSubview(mailButton)
        //データの取得
        number4Ref.observeSingleEvent(of: .value, with: { (snapshot) in self.number = snapshot.value! as! Int
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Auth.auth().currentUser != nil {
            if self.observing == false {
                ...
                let postsRef = Database.database().reference().child("Room4")
                ...
                numberRef.observe(.childChanged, with: { snapshot in
                    print("DEBUG_PRINT: .childChangedイベントが発生しました。")
                    //データの取得
                    self.number4Ref.observeSingleEvent(of: .value, with: { (snapshot) in self.number = snapshot.value! as! Int
                        print("DEBUG_PRINT: .observeSingleEventイベントが発生しました。")
                        //表示
